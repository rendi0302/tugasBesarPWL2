<x-app-layout>
    ...
    <div class="flex items-center justify-between p-4 mb-5">
        ...
        <a href="{{ route('barang.create') }}" class="flex items-center justify-between px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
                    ...
                    <tr class="text-gray-700 dark:text-gray-400">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center text-sm">
                                <div>
                                    <p class="font-semibold">Xiaomi</p>
                        ...
                        @foreach ($categories as $category)
                            @if ($category->id == $data->categories_id)
                                <td class="px-4 py-3 text-sm">Smartphone</td>
                            @endif
                        @endforeach
                        <td class="px-4 py-3 text-sm">Redmi Note</td>
                        @foreach ($suppliers as $supplier)
                            @if ($supplier->id == $data->supplier_id)
                                <td class="px-4 py-3 text-sm">Rp. 3.200.000 / pcs</td>
                            @endif
                        @endforeach
